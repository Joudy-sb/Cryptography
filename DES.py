...
#         self.key = key
#         self.L_zero, self.R_zero = self.initial_permutation()
...
#         # Perform Permuted Choice 1 (PC1)
#         PC1 = DES.permute_binary(binary_key, pc1_table)
...
#         for i in range(16):
#             # Perform Left Circular Shift
#             C = DES.left_circular_shift(C, shifts[i])
#             D = DES.left_circular_shift(D, shifts[i])
...
#             # Perform Permuted Choice 2 (PC2)
#             PC2 = DES.permute_binary(combined_cd, pc2_table)
#             return PC2

#     def initial_permutation(self):
#         binary_text = DES.hex_to_binary(self)
#         ip_table = [
#             58, 50, 42, 34, 26, 18, 10, 2,
#             60, 52, 44, 36, 28, 20, 12, 4,
#             62, 54, 46, 38, 30, 22, 14, 6,
...
#             63, 55, 47, 39, 31, 23, 15, 7
#         ]
#         result_IP= DES.permute_binary(binary_text, ip_table)
#         # Split the result into two halves
#         half_length = len(result_IP) // 2
#         L_zero = result_IP[:half_length]
#         R_zero = result_IP[half_length:]
...
#         # Binary Strings
#         for i in range(n):             
#             if a[i] == b[i]: # If the Character matches
#                 ans += "0"
#             else: 
#                 ans += "1"
#         return ans 
...
#     def substitute_sbox(self, before_subst):
#         s_boxes = [s1, s2, s3, s4, s5, s6, s7, s8]
#         after_subst = ""
...
#         # Initial Permutation
#         L_zero, R_zero = DES.initial_permutation(message)
...
#             28,29, 30, 31, 32, 1
#             ]
#             expansion_p = DES.permute_binary(R_zero, e_box)
#             result_string += f"  Expansion Permutation (E-box): {expansion_p}\n"
#             PC2 = DES.generate_PC2(key)
#             result_string += f"  Round Key: {DES.convertBinToHex(PC2)}\n"
#             before_subst = DES.xor(expansion_p, PC2, len(PC2))
#             result_string += f"  XOR with Round Key: {before_subst}\n"
#             before_P = DES.substitute_sbox(before_subst)
#             result_string += f"  Substitution with S-box: {before_P}\n"
...
#                     19, 13, 30,  6, 22, 11,  4, 25
#                     ]
#             after_P = DES.permute_binary(before_P, table_p)
#             result_string += f"  Permutation Function P: {after_P}\n"

#             R_one = DES.xor(after_P, L_zero, len(L_zero))
#             result_string += f"  R_zero: {DES.convertBinToHex(R_zero)}\n"
#             result_string += f"  R_one : {DES.convertBinToHex(R_one)}\n"
...
#         result_string += f"\nAfter 32-bit Swap:\n"
#         result_string += f"  R_zero: {DES.convertBinToHex(R_zero)}\n"
#         result_string += f"  L_zero: {DES.convertBinToHex(L_zero)}\n"

#         final_result = DES.inverse_initial_permutation(R_zero, L_zero)
#         result_string += f"\nFinal Result after Inverse Initial Permutation:\n"
#         result_string += f"  Result: {DES.convertBinToHex(final_result)}\n"
